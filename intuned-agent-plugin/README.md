...
- **`intuned`** on `PATH`, signed in. Install it with `npm install -g
@intuned/cli`, then run `intuned auth login`. All AI browser tools
...
intuned install plugin

# launch Claude Code with it
claude --plugin-dir ./intuned-plugin
```

`intuned install plugin` clones this plugin into the current directory (use
`-o <path>` to install elsewhere) and prints the exact `--plugin-dir` to use.

On session start the plugin runs `intuned dev agent-hooks setup`, which writes
...
`intuned dev agent-hooks setup` materializes the Intuned agent CLI hooks into
