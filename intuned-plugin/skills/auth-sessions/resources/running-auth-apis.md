...
| Command                                                                                  | Description                       |
| ---------------------------------------------------------------------------------------- | --------------------------------- |
| `intuned dev attempt authsession create <params> --id <id> --cdp-browser-name default`   | Login flow + verify. Saves state. |
| `intuned dev attempt authsession check <id> --cdp-browser-name default`                  | Single check attempt. No retries. |
| `intuned dev attempt api <name> <params> --auth-session <id> --cdp-browser-name default` | Run API with auth state injected. |
...
export MODE=generate_code && intuned dev attempt authsession create .parameters/auth-sessions/create/default.json --id <auth-session-id> --cdp-browser-name default --cdp-tab-id <tab_id>

# 2. Check - Runs the Check API.
export MODE=generate_code && intuned dev attempt authsession check <auth-session-id> --cdp-browser-name default --cdp-tab-id <tab_id>
```

## Running APIs with Auth

```bash
export MODE=generate_code && intuned dev attempt api <api-name> .parameters/api/<api-name>/default.json \
