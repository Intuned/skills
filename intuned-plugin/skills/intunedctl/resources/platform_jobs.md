...
- `intuned platform jobs create <data> [options]`:
  ...
  **Example — create a job from an inline JSON string:**
  ...
  ```bash
  intuned platform jobs create '{"id":"my-job","payload":[{"apiName":"get-data","parameters":{}}],"configuration":{"retry":{"maximumAttempts":3}},"schedule":{"calendars":[{"hour":12,"minute":51,"dayOfWeek":["MONDAY","TUESDAY","WEDNESDAY","THURSDAY","FRIDAY"]}],"jitter":"6h"}}'
  ```

  **Example — create a job from a file:**

  ```bash
  intuned platform jobs create jobs/my-job.job.json
  ```
...
- `intuned platform jobs update <job-id> <data> [options]`:
  - Update an existing automation job. **API-origin Jobs only.** For Code-origin Jobs, edit the `*.job.json` file and redeploy with `intuned dev deploy`.
  ...
  **Examples:**
  ...
  ```bash
  intuned platform jobs update my-job ./jobs/my-job.job.json
  intuned platform jobs update my-job '{"configuration":{"retry":{"maximumAttempts":5}}}'
  ```
...
- `intuned platform jobs delete <job-id> [options]`:
  - Delete a job. **API-origin Jobs only.** For Code-origin Jobs, remove the `*.job.json` file and redeploy with `intuned dev deploy`. Asks for an interactive confirmation prompt before deleting; pass `-y` / `--yes` to skip.
